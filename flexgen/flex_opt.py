...
from flexgen.utils import (Task, ExecutionEnv, GB, MB, T, ValueHolder,
...
from parallel_utils.utils import divide
...
class Policy:
    ...
    def update_percent(self,wg,wc,cg,cc,hg,hc):
        self.w_gpu_percent = wg
        self.w_cpu_percent = wc
        self.cache_gpu_percent = cg
        self.cache_cpu_percent = cc
        self.act_gpu_percent = hg
        self.act_cpu_percent = hc
...
def move_weight(weight_home,policy,env,reserve_dev=None):
    if reserve_dev:
        for i in range(len(weight_home.val)):
            weight_home.val[i] = weight_home.val[i].move(reserve_dev)
    else:
        dev_percents = [policy.w_disk_percent, policy.w_cpu_percent, policy.w_gpu_percent]
        dev_choices = [env.disk, env.cpu, env.gpu]
        sizes = [np.prod(weight.shape) for weight in weight_home.val]
        ...
        for i in range(len(weight_home.val)):
            ...
            if compress:
                weight_specs = weight.weight_specs
                shape, dtype, filename, start_idx, end_idx, dim = weight_specs[0], weight_specs[1], \
                                                          weight_specs[2], weight_specs[3], \
                                                          weight_specs[4], weight_specs[5]
                new_weight = home.compressed_device.allocate(
                    shape, dtype, policy.comp_weight_config, pin_memory=pin_memory)
                new_weight.shape_by_data(start_idx, end_idx, dim,weight_specs)
                new_weight.load_from_np_file(filename)
                weight_home.val[i] = new_weight
                weight.delete()
            else:
                weight_home.val[i] = weight_home.val[i].move(home)

def init_weight_list(weight_specs, policy, env, reserve_dev=None):
    dev_percents = [policy.w_disk_percent, policy.w_cpu_percent, policy.w_gpu_percent]
    dev_choices = [env.disk, env.cpu, env.gpu]
    ...
    for i in range(len(weight_specs)):
        shape, dtype, filename, start_idx, end_idx, dim = weight_specs[i][0], weight_specs[i][1], \
                                                          weight_specs[i][2], weight_specs[i][3], \
                                                          weight_specs[i][4], weight_specs[i][5]
        if not reserve_dev:
            mid_percent = (sizes_cumsum[i] - sizes[i] / 2) / sizes_cumsum[-1]
            home = get_choice(mid_percent * 100, dev_percents, dev_choices)
        else:
        ...
        if not compress:
            ...
            weight.shape_by_data(start_idx, end_idx, dim,weight_specs[i])
            weight.weight_specs = weight_specs[i]
...
class InputEmbed:
    def __init__(self, config, env, policy):
        ...
        if self.config.is_distributed:
            ...
            if self.tp_size > 1:
                p_vocab_size = self.config.vocab_size
                self.vocab_start_index, self.vocab_end_index = (
                    vocab_range_from_global_vocab_size(
                        p_vocab_size, get_tensor_model_parallel_rank(),
                        self.tp_size))
        self.task = None
    ...
    def init_weight(self, weight_home, path, reserve_dev=None):
        v, h, s, dtype = (self.config.vocab_size, self.config.input_dim,
            self.config.max_seq_len, self.config.dtype)
        path = os.path.join(path, "")
        start_idx, end_idx, dim =  -1, -1, -1
        weight_specs = [
            # w_token
            [(v, h), dtype, path + "decoder.embed_tokens.weight", start_idx, end_idx, dim],
            # w_pos
            [(s + 2, h), dtype, path + "decoder.embed_positions.weight", start_idx, end_idx, dim],
        ]
        
        if self.tp_size > 1:
            shard_size = v // self.tp_size
            start_idx = self.tensor_model_parallel_rank * shard_size
            end_idx = (self.tensor_model_parallel_rank + 1) * shard_size
            shape1, shape2 = weight_specs[0][0]
            weight_specs[0][0] = (shard_size,shape2)
            weight_specs[0][-3], weight_specs[0][-2], weight_specs[0][-1] = start_idx, end_idx, 0

        weights = init_weight_list(weight_specs, self.policy, self.env, reserve_dev)
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        input_mask = None
        if self.tp_size > 1:
            input_mask = (h.data < self.vocab_start_index) | \
                         (h.data >= self.vocab_end_index)
            masked_input = h.data.clone() - self.vocab_start_index
        ...
        h = self.compute.opt_input_embed(h, mask,
            w_token, w_pos, self.config.pad_token_id, donate, self.tp_size, input_mask)
        hidden.val = h
...
class OutputEmbed:
    ...
    def init_weight(self, weight_home, path, reserve_dev=None):
        ...
        weight_specs = [
            # w_ln
            [(h,), dtype, path + "decoder.layer_norm.weight", start_idx, end_idx, dim],
            # b_ln
            [(h,), dtype, path + "decoder.layer_norm.bias", start_idx, end_idx, dim],
            # w_token
            [(v, h), dtype, path + "decoder.embed_tokens.weight", start_idx, end_idx, dim],
        ]
        if self.tp_size > 1:
            ...
            start_idx = self.tensor_model_parallel_rank * shard_size
            end_idx = (self.tensor_model_parallel_rank + 1) * shard_size
            weight_specs[2] = [(shard_size, h), dtype, path + "decoder.embed_tokens.weight", start_idx, end_idx, 0]
        weights = init_weight_list(weight_specs, self.policy, self.env, reserve_dev)
        weight_home.store(weights)
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        h = self.compute.opt_output_embed(h, w_ln, b_ln, w_token, donate,
            self.task.do_sample, self.task.temperature,self.config.vocab_size,self.tp_size)
        hidden.val = h
...
class SelfAttention:
    def __init__(self, config, env, policy, layer_id):
        ...
        assert self.config.n_head % self.tp_size == 0
        self.n_head = total_num_heads // self.tp_size
        self.embed_dim = self.config.hidden_size
        self.head_dim = self.embed_dim // total_num_heads
    ...
    def init_weight(self, weight_home, path, reserve_dev=None):
        ...
        weight_specs = [
            # w_q
            [(h, h), dtype, path + ".q_proj.weight", start_idx, end_idx, dim],
            # b_q
            [(h,), dtype, path + ".q_proj.bias", start_idx, end_idx, dim],
            # w_k
            [(h, h), dtype, path + ".k_proj.weight", start_idx, end_idx, dim],
            # b_k
            [(h,), dtype, path + ".k_proj.bias", start_idx, end_idx, dim],
            # w_v
            [(h, h), dtype, path + ".v_proj.weight", start_idx, end_idx, dim],
            # b_v
            [(h,), dtype, path + ".v_proj.bias", start_idx, end_idx, dim],
            # w_out
            [(h, h), dtype, path + ".out_proj.weight", start_idx, end_idx, dim],
            # b_out
            [(h,), dtype, path + ".out_proj.bias", start_idx, end_idx, dim],
            # w_ln
            [(h,), dtype, path + "_layer_norm.weight", start_idx, end_idx, dim],
            # b_ln
            [(h,), dtype, path + "_layer_norm.bias", start_idx, end_idx, dim],
        ]
        if self.tp_size > 1:
            for i in range(len(weight_specs)):
                shard_size = h // self.tp_size
                start_idx = self.tensor_model_parallel_rank * shard_size
                end_idx = (self.tensor_model_parallel_rank + 1) * shard_size
                if i in range(6):
                    if len(weight_specs[i][0]) > 1:
                        shape1, shape2 = weight_specs[i][0]
                        weight_specs[i][0] = (shard_size, shape2)
                    else:
                        weight_specs[i][0] = (shard_size,)
                    weight_specs[i][-3], weight_specs[i][-2], weight_specs[i][-1] = start_idx, end_idx, 0
                   
                if i == 6:  
                    # ".out_proj.weight"
                    shape1, shape2 = weight_specs[i][0]
                    weight_specs[i][0] = (shape1, shard_size)
                    weight_specs[i][-3], weight_specs[i][-2], weight_specs[i][-1] = start_idx, end_idx, 1
        weights = init_weight_list(weight_specs, self.policy, self.env, reserve_dev)
        weight_home.store(weights)
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        donate = [False] * 14
        h, donate[0] = hidden.val, True  # bsz, tgt_len, _
        ...
        if i == 0:  # prefill
            ...
            h, new_k_cache, new_v_cache = self.compute.mha(h, mask, w_q, b_q,
                ...
                self.policy.compress_cache, self.policy.comp_cache_config, self.tp_size)
...
class MLP:
    ...
    def init_weight(self, weight_home, path, reserve_dev=None):
        ...
        weight_specs = [
            # wi
            [(4 * h, h), dtype, path + "fc1.weight", start_idx, end_idx, dim],
            # bi
            [(4 * h,), dtype, path + "fc1.bias", start_idx, end_idx, dim],
            # wo
            [(h, 4 * h), dtype, path + "fc2.weight", start_idx, end_idx, dim],
            # bo
            [(h,), dtype, path + "fc2.bias", start_idx, end_idx, dim],
            # w_ln
            [(h,), dtype, path + "final_layer_norm.weight", start_idx, end_idx, dim],
            # b_ln
            [(h,), dtype, path + "final_layer_norm.bias", start_idx, end_idx, dim],
        ]
        if self.tp_size > 1:
            for i in range(len(weight_specs)):
                shard_size = 4 * h // self.tp_size
                start_idx = self.tensor_model_parallel_rank * shard_size
                end_idx = (self.tensor_model_parallel_rank + 1) * shard_size
                if i == 0: 
                    # "fc1.weight", "fc1.bias"
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        if k == self.policy.num_gpu_batches - 1:
            ...
            ((wi, _), (bi, _), (wo, _), (bo, _),
             (w_ln, _), (b_ln, _)) = weight_read_buf.val
        h = self.compute.mlp(h, wi, bi, wo, bo, w_ln, b_ln, donate, self.tp_size)
        hidden.val = h
...
class TransformerLayer:
    ...
    def init_weight(self, weight_home, path, reserve_dev=None):
        ...
        self.attention.init_weight(home1, path,reserve_dev)
        self.mlp.init_weight(home2, path,reserve_dev)
...
class OptLM:
    ...
    def init_weight(self, j, reserve_dev=None):
        ...
        self.layers[j].init_weight(self.weight_home[j], expanded_path, reserve_dev)
    ...
    def init_all_weights(self): 
        ...
        if not self.policy.by_layer:
            ...
            layer_indexes = [j for j in range(self.num_layers)]
            ...
            for gpu_layer_index in gpu_layers_indexes:
                self.init_weight(gpu_layer_index,self.env.gpu)
            for cpu_layer_index in cpu_layers_indexes:
                self.init_weight(cpu_layer_index,self.env.cpu)
            for disk_layer_index in disk_layers_indexes:
                self.init_weight(disk_layer_index,self.env.disk)
...
def run_flexgen(args):
    ...
    args.gpu_batch_size  = num_prompts // args.num_gpu_batches
    print("num_prompts, args.gpu_batch_size, args.num_gpu_batches", num_prompts, args.gpu_batch_size, args.num_gpu_batches)
...
def add_parser_arguments(parser):
    ...
    parser.add_argument("--gen-len", type=int, default=38)
    ...
    parser.add_argument("--by-layer", type=str2bool, nargs='?',
        const=False, default=False,
        help="offload by layer.")
