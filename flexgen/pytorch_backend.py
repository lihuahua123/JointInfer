...
import os
import sys
sys.path.append('/home/server/DistributedOffload/vllm/')
from parallel_utils.communication_op import (
...
from parallel_utils.parallel_state import (
    get_tensor_model_parallel_world_size, get_tensor_model_parallel_rank, get_pipeline_model_parallel_rank)
...
class TorchTensor:
    ...
    def shape_by_data(self, start_idx, end_idx, dim, weight_specs):
        # , start_idx, end_idx, dim
        # if dim == 0:
        #     weight.data = weight.data[start_idx:end_idx]
        # else:
        #     weight.data = weight.data[:, start_idx:end_idx]
    ...
    def load_from_np(self, np_array):
        if self.device.device_type == DeviceType.DISK:
            ...
            if self.start_idx > -1:
                if self.dim == 0:
                    arr = arr[self.start_idx:self.end_idx]
                else:
                    arr = arr[:, self.start_idx:self.end_idx]
            ...
            if self.device.device_type == DeviceType.COMPRESSED:
                arr = torch.from_numpy(np_array)
                if self.start_idx > -1:
                    if self.dim == 0:
                        arr = arr[self.start_idx:self.end_idx]
                    else:
                        arr = arr[:, self.start_idx:self.end_idx]
                tmp = global_cpu_device.compressed_device.compress(arr, self.data[2])
...
class TorchDevice:
    ...
    def opt_input_embed(self, inputs, attention_mask, w_token, w_pos, pad_token_id, donate, tp_size = 1, input_mask = None):
        # decompress weights
        ...
        if tp_size > 1:
            token_embed[input_mask, :] = 0.0
        # Reduce across all the model parallel GPUs.
            token_embed = tensor_model_parallel_all_reduce(token_embed)
    ...
    def opt_output_embed(self, inputs, w_ln, b_ln, w_token, donate,
                         do_sample, temperature,vocab_size, tp_size=1):
        # decompress weights
        if w_token.device.device_type == DeviceType.COMPRESSED:
            w_token = w_token.device.decompress(w_token)
        ...
        # print("tensor_model_parallel_all_gather before", torch.cuda.max_memory_allocated('cuda:0')/GB)
        if tp_size > 1:
            logits = tensor_model_parallel_all_gather(logits)
        # print("tensor_model_parallel_all_gather after", torch.cuda.max_memory_allocated('cuda:0')/GB)
    ...
    def init_cache_one_gpu_batch(self, config, task, policy):
        ...
        if tp_size > 1:
            shape = (prompt_len + gen_len - 1, gpu_batch_size * num_head // tp_size , hidden_size // num_head)
        else:
    ...
    def mha(self, inputs, attention_mask, w_q, b_q, w_k, b_k, w_v, b_v,
            w_out, b_out, w_ln, b_ln, n_head, donate, compress_cache, comp_config, tp_size=1):
        ...
        b, s, h = inputs.shape # bsz, tgt_len, _
        head_dim = h // n_head
        ...
        # print("ColumnParallelLinear", tensor_model_parallel_all_gather(output_parallel)[0][0][:2])
        # attention
        ...
        # shape: (b, n_head, s, head_dim)
        value = torch.bmm(attn_weights, v).view(b, n_head, s, head_dim) # ([24, 6, 32, 64])
        # shape: (b, s, h)
        value = value.transpose(1, 2).reshape(b, s, h) # ([24, 32, 384])
        # 在前面都没问题
        # RowParallelLinea
        value = F.linear(value, w_out.data)
        if tp_size > 1:
            value = tensor_model_parallel_all_reduce(value)
    ...
    def mha_gen(self, inputs, attention_mask, w_q, b_q, w_k, b_k, w_v, b_v,
                ...
                attn_sparsity, compress_cache, comp_config, tp_size=1):
    ...
    def mlp(self, inputs, wi, bi, wo, bo, w_ln, b_ln, donate, tp_size = 1):
        # decompress weights
...
class TorchDisk:
    ...
    def __init__(self, path, mem_capacity=None, cuda_id=0, num_copy_threads=2):
        self.name = path
    ...
    def allocate(self, shape, dtype, pin_memory=None, name=None):
        ...
        d = np.ones(shape, dtype=np.float16)
        np.lib.format.open_memmap(path, mode="w+", shape=d.shape, dtype=d.dtype)
...
class TorchMixedDevice:
    ...
    def init_cache_one_gpu_batch(self, config, task, policy):
        ...
        if tp_size > 1:
            ...
            shape = (prompt_len + gen_len - 1, gpu_batch_size * num_head, hidden_size // num_head)
        # We have to round to a multiple of `num_head` SEG_DIM=1 // 向下取整
...
def general_copy(dst: TorchTensor, dst_indices: Tuple[slice],
    ...
    if dst.device.device_type == DeviceType.MIXED:
        ...
        # The tensor is on the disk, dispatch to copy threads for asynchronous copy
        dst.device.submit_copy(dst, dst_indices, src, src_indices) # 这句出了问题
