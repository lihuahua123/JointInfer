...
def client_process(
    ...
    tokenizer = AutoTokenizer.from_pretrained("/home/server/models/OPT-125M")
...
def main():
    ...
    parser.add_argument('--num-micro-batches', default=2, type=int)
    parser.add_argument('--num-processes', default=1, type=int)
    parser.add_argument('--output', required=True)
    args = parser.parse_args()

    config_bloom = DistributedBloomConfig.from_pretrained("/home/server/models/bigscience/bloom-petals")
    ...
    for sequence_length in [32,64,128,246,512]:
        run_bench(args=args, sequence_length=32, max_tokens=sequence_length, config_bloom=config_bloom)
    #for batch_size in [1,2,4,8,16,32]:
    #     args.batch_size = batch_size
    #     run_bench(args=args, sequence_length=32, max_tokens=128, config_bloom=config_bloom)
...
def run_bench(args, sequence_length, max_tokens, config_bloom):
    ...
    #np.save(f'./single_{max_tokens}',latencies)
    print("average latency", latency)
    #np.save(args.output,[total_time])
