...
i=0
by_layer=False
log_file="one_node_flexgen_$pp-$tp-$w_gpu-$w_cpu-$c_gpu-$c_cpu_$4"
if [ $word_size -lt 5 ]; 
    then
    while [ $i -lt $word_size ] 
    do
    local_rank_i=`expr $i + 2`
    $PYTHON_EXEC $script $tensor_parallelism --log-file $log_file $compress --memory-fraction $memory_fraction --by-layer $by_layer --head-ip $master_host --port 7677 --world-size $word_size --rank $i --local-rank $local_rank_i --model $model --local --percent $w_gpu $w_cpu $c_gpu $c_cpu $a_gpu $a_cpu --comm-device gpu --gen-len  $gen_len --prompt-len $prompt_len --num-prompts $num_prompts --pp $pp --tp $tp --overlap $overlap --async-comm --num-inner-iterations $num_inner_iterations &
    let i++ 
    done
else
    while [ $i -lt 4 ] 
