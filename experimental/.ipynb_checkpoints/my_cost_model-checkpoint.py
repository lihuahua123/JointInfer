...
[1.1399717926979065,549.1055432409048,0.32256897538900375,135.71662835031748,389.69594582542777,23.86287409812212,551.1319472081959]
...
devs = [VM(0,'V100',1,16 * GB,204 * GB,500 * GB,484.639 * GB,105.84),
       ...
       VM(1,'V100',2,16 * GB,204 * GB,500 * GB,484.639 * GB,105.84),
       VM(1,'V100',2,16 * GB,204 * GB,500 * GB,484.639 * GB,105.84),
       VM(2,'V100',3,16 * GB,204 * GB,500 * GB,484.639 * GB,105.84),]
...
def get_best_dev(devs,layer_num=1,pre=None): # 层数和前面的 TODO
    tp_dev = devs[0]
...
def DP(k,j,devs,opt_config): # 分成j个stage，前k层给前j个stage
    ...
    if j == 1:
        ...
        return best_dev,devs.remove(best_dev[0][0])
    for i in range(1,k):
        if j-1 > i:
            continue
#         new_devs = devs.copy()
#         tp_dev = get_best_dev(new_devs)
#         new_devs.remove(tp_dev)   
        pre,left_devs = DP(i,j-1,new_devs,opt_config)
        tp_dev = get_best_dev(left_devs)
...
#     dev = DP(opt_config.num_hidden_layers,j,devs,opt_config)
#     latency = dev[-1]
#     if latency < min_latency:
#         min_latency = latency # s 来计算 延迟计算错误 因为带宽单位是GB忘记乘以GB了，怎么体现优越性
#         best_dev = dev
#         stage_num = j
# end=time.time()
# print(end-begin,min_latency,best_dev,stage_num)
# -
