...
def init_cost_config(num_layer,opt_config):
    ...
    return config
def get_cache_percentage(wg,wc,wn,num_layer,tp,opt_config): # TODO:cache 的tp
        ...
        for hold_layer_num in range(1,layers_num+1): # 这个假设成线性
            ...
            for i in  range(0,len(deduplicated_percentages)):
                ...
                for j in range(i,len(deduplicated_percentages)):
                    ...
                    cache_percentage = get_cache_percentage(gpu_percent, cpu_percent,disk_percent,hold_layer_num,tp,opt_config)
...
#                             cpu_percent = 1 - gpu_percent
            ...
            if len(profile_setting[tp][hold_layer_num])>1:
                profile_setting[tp][hold_layer_num]=profile_setting[tp][hold_layer_num][1:]
    return profile_setting
# model_name='/home/server/models/OPT-30B'
# name = model_name.split("/")[-1]
# p = get_setting(model_name)
# tps = [1,2,4]
...
for tp in tps:
    ...
    for a in aa:
        kk[a[0]][a[1]].append(a[2:])
def get_computing_latency(tp_dev,pp_layers): # per token
...
def DP(k,j,devs,opt_config): # 分成j个stage，前k层给前j个stage
    ...
    for i in range(1,k):
        ...
        for tp_dev in devs:
            ...
            all_latency = pre[-1] + (k-i) * tp_latency + pp_latency + min_profile[-2] * cost_config.s +min_profile[-1] * cost_config.n
            if all_latency < min_latency:
                min_latency = all_latency
                pre[0].append(tp_dev)
