...
class CostModelConfig:
    ...
    cmem: int = 204 * GB
    nmem: int = 1500 * GB
    # (prompt_len = 32 gen_len = 30, gpu_batch_size = 4, num_head = nh, hidden_size=h1, 
    # if tp_size > 1:
    ...
    ctog_bdw: float = 12.89 * GB # CPU 2 GPU 带宽
    gtoc_bdw_cache: float = 10.97 * GB # GPU2CPU cache 带宽
    gtoc_bdw_hidden: float = 4.82 * GB # GPU2CPU hidden 带宽

    dtoc_bdw: float = 0.473 * GB
    ctod_bdw_cache_p: float = 0.746 * GB
    ctod_bdw_hidden_p: float = 2.015 * GB
    ctod_bdw_g: float = 2.015 * GB # 为什么generate 没有cache 和 hidden

    mm_flops_p: float = 21.24 * T # prefill
    mm_flops_g: float = 4.3 * T # gene
...
def solve_lp(config, bls, gbs, compress_w=False, verbose=0, debug=False, percent=None):
    ...
    policy = Policy(gbs, bls // gbs,
                    ...
                    overlap=True, sep_layer=False, pin_weight=False,by_layer=False,
...
if __name__ == "__main__":
    ...
    # python cost_model.py --model facebook/opt-30b --prompt-len 32 --gen-len 512 --layer_num 12 --gbs 4 --num-gb 1
    # python cost_model.py --compress-w --model facebook/opt-30b --prompt-len 32 --gen-len 512 --layer_num 12 --gbs 4 --num-gb 1 --gpu-mem 8
    opt_config = get_opt_config(args.model)
    config.l = args.layer_num #opt_config.num_hidden_layers
    config.h1 = opt_config.hidden_size
    config.h2 = opt_config.ffn_embed_dim
    config.nh = opt_config.n_head
    config.s = args.prompt_len
